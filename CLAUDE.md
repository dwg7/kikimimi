...
- [ ] 十勝岳関連の実ニュース音声での固有名詞転記精度の確認。**継続稼働
      ...
      「「十勝」自体が誤転記された実例」節)。**large-v3-turboへの本番
      切り替え+既存1147件の遡り再文字起こし完了(2026-09-20)。**
      旭川・岩見沢・帯広等の周辺地名は改善したが、「十勝」漢字表記は
      backfill後も一度も正しく出現せず(とか/塚/トカチの誤転記が継続)。
      ただしいずれも通常の天気予報の区域名への言及で、火山名「十勝岳」
      への言及ではない。実証された誤転記パターンのうちリスクの低い
      「トカチ」(カタカナ)をtokachi-lensのgate.txtに追加した
      (2026-09-21、ユーザー承認)
      ([ADR 0011](documents/decisions/0011-transcription-moves-to-macmini-role.md)
      「追記: large-v3-turboへの本番切り替えと、既存1147件の遡り
      再文字起こし」節)
- [x] LLM判定パイプラインの生存確認用に、gate.txtを持たない
      `ja-radio-disaster`レンズ(OpenSpeechMap本体付属)を、`tokachi-lens`
      とは独立したタイマー(10分間隔)・独立した出力先で並行稼働させた
      (2026-09-20)。実機検証(1373件処理、35件がis_disaster=trueと
      判定)で、台風25号関連の実際の報道時間帯(07時台・12時台・19時台の
      定時ニュース)に正しく反応することを確認し、tokachi-lensが
      ずっと0件なのは判定経路の不調ではなく実際に対象事象が無いためだ、
