...
### RTL-SDR個体のPPM較正(2026-09-20)

GNSS受信の検討(ADR 0013参照)から派生して、手元のRTL-SDR Blog V4
個体固有の周波数誤差(PPM)を、既知の正確な周波数を持つ信号
(NHK-FM北海道、85.2MHz)を基準に実測した。

`rtl_power -f 85.0M:85.4M:50 -i 60 -e 60`を20分間で8回繰り返し、
2つの手法で解析した。

| 手法 | 平均 | 標準偏差 |
|---|---|---|
| 最大値法(ピークのビンをそのまま使う) | -0.84 ppm | 1.30 ppm |
| パワー重み付き重心法 | -4.73 ppm | 7.37 ppm |

**最大値法の方が明らかに安定していた**(8回中6回が-1.06〜+0.09ppmに
収まる)。重心法は帯域内のノイズフロア推定が隣接周波数の混入等に
弱く、2回(-17.41ppm、-14.69ppm)の大きな外れ値を出し、実用に
耐えないことが分かった。

**結論**: この個体のPPM誤差は概ね**-0.8ppm前後**と推定される。
V4の公称スペック(1PPM TCXO、初期オフセット±2ppm・温度ドリフト
±1ppm)の範囲内に収まっており、個体として健全。今回のスキャンの
周波数分解能(約48.8Hz≒0.57ppm/bin)が測定のばらつきの主因である
可能性が高く、実際の誤差はさらに小さい可能性もある。将来
`rtl_fm`/`rtl_power`系コマンドで`-p`(PPM補正)オプションを使う際の
参考値として記録しておく。
