...
## 追記: USB再列挙による無応答障害(2026-09-20)

継続稼働2日目、アンテナ位置調整(受信環境確認のため屋外へ一時移動)を
行った際、**`kikimimi-record.service`が「動いてはいるが、新しいデータを
一切生成しない」状態に陥る**障害が見つかった。

- 07:43:57を最後に新しいセグメントが生成されなくなり、約13分間
  停止した(RPi実機のログで確認)
- `rtl_fm`・`ffmpeg`プロセスはクラッシュせず稼働し続けていた
  (`ps aux`でPID・起動時刻が変わっていないことを確認)
- 一方`rtl_test`は問題なくデバイスを検出できていた

**推定原因**: アンテナ・USBケーブル周りの物理的な操作で、RTL-SDR
デバイスのUSB接続が瞬断→再列挙され、既存の`rtl_fm`プロセスが
古い(もう存在しない)デバイスハンドルを掴んだまま無反応になった
と考えられる。プロセス自体は生きているため、`record.py`の
`supervise()`が前提とする「プロセスが終了したら再起動する」という
監視ロジックでは検知できない障害である(`record.py`のdocstringにある
「ffmpeg does the recording...keep it running when it dies」が
想定していない、「死なずに無応答になる」ケース)。

**対処**: `sudo systemctl restart kikimimi-record.service`で復旧を
確認した。恒久対処(例: 一定時間セグメントが増えない場合に外部から
再起動するウォッチドッグ)は未実装——今回は手動対処に留め、次回
同種の障害が頻発するようなら`scripts/diagnose.sh`等への組み込みを
検討する。

**教訓**: アンテナ・USBケーブル周りを物理的に触った後は、
`ls /mnt/kikimimi-audio/`でセグメントが実際に増え続けているかを
確認する習慣が必要(プロセスが生きていることだけでは不十分)。

## 追記: 1個の壊れたファイルが後続すべてを止める障害(2026-09-20)

アンテナ・ゲイン調整のため`kikimimi-record.service`を何度も
stop/startした際、**セグメント境界とちょうど重なるタイミングで
0バイトの壊れた`.ts`ファイルが生成される**ことがあった
(`NHKFM-20260920-082019.ts`)。

`speechmap transcribe`は入力ディレクトリのファイルを時刻順に処理するが、
`ffprobe`が失敗するファイルに当たると例外を送出し、**それ以降のファイル
(時刻的に後、つまり正常なファイルも含む)を一切処理しない**設計に
なっている(`duration_of()`の`TranscribeError`がループ内で捕捉されて
いない)。この結果、08:20頃に発生した1個の壊れたファイルが原因で、
**08:20から11:38まで約3時間分、正常な音声の文字起こしが完全に
止まっていた**(録音・rsync同期自体は正常に継続していたため、
音声データそのものは失われていない)。

また、壊れたファイルはRPi側では(record.pyの`--retention-hours`の
掃除で)既に消えていたが、**作業用Mac側のローカルコピーには残り
続けていた**——rsyncは`--delete`を付けない限り、送信元で消えた
ファイルを受信側から消さないため。

**対処**: 該当ファイルを`~/kikimimi-audio-quarantine/`へ退避
(削除はしていない)し、`sync-segments.sh`を手動実行して滞留を
解消した。全ファイルを`ffprobe`で検査して壊れたものを一括で
見つける、その場しのぎのワンライナーを使った(恒久的なツール化は
まだ)。

**今後の対処案(未実装)**: `sync-segments.sh`または`speechmap
transcribe`側で、1ファイルの失敗を検知したらログに残しつつ
次のファイルへ進む(壊れたファイルだけスキップする)ロジックが
あれば、今回のような長時間の停止を防げる。OpenSpeechMap本体側の
挙動を変える話になるため、上流に提案するか、`sync-segments.sh`側で
「壊れているファイルを自動でquarantineディレクトリへどける」
前処理を挟むか、次回検討する。
